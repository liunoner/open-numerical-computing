...
function renderDetail(name){
  ...
  const sectionIndex=sections.findIndex(s=>s.id===placement.sectionId);
  const section=sections[sectionIndex];
  const item=section&&section.items.find(x=>x.title===placement.itemTitle);
  if(sectionIndex>=0){
    active=sectionIndex;
    currentItem=Math.max(0,section.items.findIndex(x=>x.title===placement.itemTitle));
    expandedIndex=active;
  }
  drawNav();
  ...
  const modal='<div class="detail-modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="detail-modal-title"><div class="detail-modal"><button class="detail-modal-close" type="button" aria-label="关闭提示窗口">×</button><p class="detail-modal-kicker">DETAIL PAGE NOTICE</p><h2 id="detail-modal-title">该项目详情页仍在完善中</h2><p>该项目详情页的完整内容尚未完成。当前已收录的项目概览、源码与社区等资料可以正常浏览，其余内容将持续补充与核验。</p><div class="detail-modal-note">关闭此提示后，可正常浏览当前详情页中的全部现有内容。</div></div></div>';
